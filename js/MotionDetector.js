...
window.ENABLE_BLEND_THRESH = true;
window.BLEND_THRESH = 150; // [0, 255]
...
var MotionDetector = (function() {
  ...
  var energyConsole = document.getElementById('energyConsole')
    ...
    if (localStream) {
      ...
      while (x < length) {
        if (!greyScale) {
          ...
          if (window.ENABLE_BLEND_THRESH) {
            blended = (blended > window.BLEND_THRESH) ? 255 : 0;
          }
      ...
      //console.log(imgData.data.reduce((a, b) => a + b, 0) / (255 * imgData.data.length))
      // 307000 depends on the canvas size in pixels
      //console.log((countOccurrences(imgData.data, 255) - 307000) / 100)
      const energyScore = (countOccurrences(imgData.data, 255) - 307000) / 100;
